...
![Defective input, completion by the CD + repulsion model, and ground truth for validation skull 039](reports/figures/qualitative_completion_case039.png)

*Validation skull 039, fold 0: (a) the defective input, (b) the completion, (c) the
ground truth. All three surfaces are reconstructed from point clouds the same way.*
...
Three things are git-ignored: `experiments/`, the trained checkpoints, which are
on Google Drive (see [Checkpoints](#checkpoints)); `msn_downloads/`, the upstream
released weights; and `data/14161307/`, the raw SkullFix volumes, which are not
redistributed here.
...
| loss | runs |
|---|---|
| CD | `cd_only_f0` ... `cd_only_f4` |
| CD + DCD | `lr_fix_only_f0` ... `lr_fix_only_f4` |
| CD + DCD + repulsion | `rep_w05_f0` ... `rep_w05_f4` |
| CD + repulsion | `cd_rep05_full_f0` ... `cd_rep05_full_f4` |

Each run folder holds `best.h5` (750 MB), `run.json` and `history.csv`; 15 GB in
total. `MD5SUMS.txt` in the Drive folder verifies the downloads. Keep the layout:

```
experiments/msn_skullfix/cd_rep05_full_f0/best.h5
```

Only inference, the surface figures and metric recomputation need them. The
results tables read `experiments_log/` and do not.
